...
const ProfileInfo = () => {
    ...
    const userFields = [
        { label: 'PHOTO', value: <img src={user.photo} alt='User Photo' className='w-16 h-16 rounded-full' /> },
        ...
        <div className='flex flex-col items-center justify-center my-4 lg:border lg:rounded-lg lg:w-7/12 lg:h-auto'>
            <div className='flex flex-row justify-between items-center gap-4 w-full p-4 border-b'>
                <div className='flex flex-col items-start justify-center w-2/3 my-2'>
                    ...
                    <p className='text-xs text-gray-500'>Some info may be visible to other people</p>
                </div>
                <button className='text-gray-500 text-sm px-7 py-1 border rounded-md border-gray-500'>Edit</button>
            </div>
            <div className='w-full'>
                {userFields.map((field, index) => (
                    <div key={index} className='flex flex-row justify-between items-center gap-4 w-full p-4 border-b'>
                        <div className='flex flex-col items-start justify-center w-1/3'>
                            <h3 className='text-xs font-semibold text-gray-400'>{field.label}</h3>
                        </div>
